/*
 * ExtremelyRandomizedTrees.h
 *
 *  Created on: 28 янв. 2016 г.
 *      Author: lladmin
 */

#ifndef MODELS_EXTREMELYRANDOMIZEDTREES_H_
#define MODELS_EXTREMELYRANDOMIZEDTREES_H_
...
class ExtremelyRandomizedTrees: public BaseEnsembleClassifier{
protected:
...
public:
		...
		ExtremelyRandomizedTrees(unsigned int randomSeed){
		...
		void setMtryAndSplitsNumber(BaseClassifier* bc){
			ExtremelyRandomizedTree * t = static_cast<ExtremelyRandomizedTree*>(bc);
			RandomFeaturesSplitCandidateGenerator<RandomSplitCandidateGenerator> *sg =
					static_cast<RandomFeaturesSplitCandidateGenerator<RandomSplitCandidateGenerator> *>(t->getSplitGenerator());
		...
		virtual void trainNewClassifier(Dataset *d){
			...
			IndexVectorUniquePtr vec = r.getRandomIndexVector(trainSubsetSize, 0, d->getNumSamples() - 1, true);
			DatasetUniquePtr data(new Dataset);
			data = std::move( d->getSubset(vec.get()) );

			BaseClassifierUniquePtr classifier(new ExtremelyRandomizedTree);
...
#endif /* MODELS_EXTREMELYRANDOMIZEDTREES_H_ */
